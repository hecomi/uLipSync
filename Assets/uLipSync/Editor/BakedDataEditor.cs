...
namespace uLipSync
{
    ...
    public class BakedDataEditor : Editor
    {
        ...
        void DrawFrames(Rect rect)
        {
            ...
            phonemeList = data.isSparse ? data.phonemes : data.bakedProfile.GetPhonemeNames().ToList();
            var phonemeCount = phonemeList.Count;
            ...
            for (int j = 0; j < phonemeCount; ++j)
            {
                for (int i = 0; i < n; ++i)
                {
                    ...
                    if (data.isSparse)
                    {
                        val = GetRatio(frame, phonemeList[j]) * frame.volume;
                    }
                    else
                    {
                        val = frame.phonemes[j].ratio * frame.volume;
                    }
            ...
            for (int j = 0; j < phonemeCount; ++j)
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    if (data.isSparse)
                    {
                        y = GetRatio(frame, phonemeList[j]) * frame.volume / maxRatio;
                    }
                    else
                    {
                        y = frame.phonemes[j].ratio * frame.volume / maxRatio;
                    }
            ...
            // Update legend to use phonemeList
        ...
        {
            return (from pr in frame.phonemes where pr.phoneme == phoneme select pr.ratio).FirstOrDefault();
        }
        ...
        public void Bake()
        {
            ...
            data.frames.Clear();
            data.phonemes = new List<string>(data.profile.GetPhonemeNames());
            data.isSparse = true;
            ...
            for (int offset = 0; offset < buffer.Length - samplePerFrame; offset += samplePerFrame)
            {
                ...
                BakedFrame frame = new()
                {
                    volume = ls.result.rawVolume,
                    phonemes = new List<BakedPhonemeRatio>()
                };

                foreach (KeyValuePair<string, float> kv in ls.result.phonemeRatios.Where(kv => kv.Value > 0f))
                {
                    frame.phonemes.Add(new BakedPhonemeRatio { phoneme = kv.Key, ratio = kv.Value });
                }
